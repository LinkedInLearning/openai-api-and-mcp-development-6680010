## Install and use an MCP Server as a Visual Studio Code extension  

======================================================
### Powered by GitHub Models, Actions, and the Model Context Protocol (MCP)

This repository guides you through **installing, configuring, and running an MCP (Model Context Protocol) server** and **connecting it to Visual Studio Code as an MCP extension**.

With MCP, Visual Studio Code (via an MCP-compatible extension) can:
- Discover tools exposed by your MCP server
- Invoke those tools directly from the editor
- Use **GitHub Models** (or OpenAI-compatible models) as the LLM backend
- Safely access project context, APIs, and workflows
...
- An OpenAI account (optional, OpenAI-compatible)
...
\[6\] - **Register the MCP Server in VS Code**
...
#### 6.2 Open VS Code Settings (JSON)

Open **Command Palette** (`Cmd/Ctrl + Shift + P`) and select:
...
* Automatically launches the MCP server from VS Code

* Injects secrets securely using environment variables

* Registers MCP tools so they are available inside the editor


#### Next Steps :
* Restart VS Code
Restart VS Code to allow the MCP extension to discover and connect to the server.

* Verify the VS Code ↔ MCP Connection
